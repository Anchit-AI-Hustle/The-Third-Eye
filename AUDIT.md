...
|---|---|---|---|---|---|---|---|---|
| F-01 | P0 | Pending | `frontend/src/lib/auth.ts:7`, `frontend/src/lib/api.ts` | No backend deployment exists; the one integration point fails silently; the client is imported nowhere | Live criteria #3/#4 are false; the entire `backend/` investment is disconnected from the product | **Decided 2026-08-22: (B) deploy.** Scoped in PROJECT_STATUS.md — Cloud Run + Cloud SQL + Memorystore, standalone first (frontend cutover is a separate later decision). Real ongoing GCP spend; awaiting cost go-ahead before provisioning | Low for standing it up alone; cutover (if pursued later) is higher | XL, not started |
