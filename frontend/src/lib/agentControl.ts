...
export function describeSideEffect(type: string, data?: Record<string, any>): string {
  ...
  switch (type) {
    ...
    case "expense_create": return `Logged expense ₹${d.amount ?? ""}${d.category ? ` · ${d.category}` : ""}`;
    case "expense_delete": return `Deleted an expense`;
    ...
    case "device_action": return `Device: ${d.action ?? ""} ${d.device ?? ""}`.trim();
    case "home_action": return `Home Hub: ${d.action ?? ""} ${d.device ?? ""}`.trim();
    case "batch": return "Ran a protocol batch";
    default: return type;
  }
}
