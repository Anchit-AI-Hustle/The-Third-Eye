...
const AUDITS: Audit[] = [
  {
    ...
    current: 6.0,
    target: 9.5,
    dims: { accuracy: 6, implementation: 6, execution: 6, results: 6 },
    verdict:
      "Clean shell and good realtime intent, but data only flows when Supabase RLS lets the anon browser client through — which it does not by default. A duplicated task list and unguarded empty states drag it down.",
    issues: [
      { severity: "critical", text: "Browser Supabase client uses the anon key with no NextAuth→Supabase bridge, so RLS blocks reads/writes once Supabase is configured.", fix: "Bridge NextAuth identity into Supabase (signed JWT with the user claim) or move reads/writes to server routes using the service role.", status: "pending" },
  ...
  },
  {
    ...
    current: 4.5,
    target: 9.5,
    dims: { accuracy: 5, implementation: 4, execution: 5, results: 4 },
    verdict:
      "Functionally the weakest of the core pages. The schema is missing columns the UI references, and writes silently fail under RLS. Auto-track vs. manual-add is not clearly separated.",
    issues: [
      { severity: "critical", text: "tasks table lacks assignee / start_date / completed_at columns that the UI reads and writes.", fix: "Add the missing columns via migration (supabase-schema); default nullable.", status: "pending" },
      { severity: "critical", text: "Writes go through the anon client and fail under RLS with no surfaced error.", fix: "Same auth-bridge fix as Dashboard; surface write failures to the user.", status: "pending" },
  ...
  },
  {
    ...
    current: 5.0,
    target: 9.5,
    dims: { accuracy: 5, implementation: 5, execution: 5, results: 5 },
    verdict: "Basic capture works locally but persistence is unreliable and there's no debounce on autosave.",
    issues: [
      { severity: "high", text: "Same anon/RLS persistence gap — notes don't reliably survive a reload when Supabase is on.", fix: "Auth-bridge fix.", status: "pending" },
      { severity: "medium", text: "Autosave writes on every keystroke.", fix: "Debounce autosave (~500ms).", status: "pending" },
    ],
  },
  {
    page: "Goals",
    ...
    target: 9.5,
    dims: { accuracy: 5, implementation: 5, execution: 5, results: 5 },
    verdict: "Progress maths can divide by zero and persistence has the same RLS gap.",
    issues: [
      { severity: "high", text: "Division by zero when a goal has no milestones → NaN% progress.", fix: "Guard the denominator; show 0% for empty goals.", status: "pending" },
      { severity: "high", text: "Anon/RLS persistence gap.", fix: "Auth-bridge fix.", status: "pending" },
    ],
  },
  {
    page: "Knowledge",
    ...
    target: 9.5,
    dims: { accuracy: 6, implementation: 6, execution: 6, results: 6 },
    verdict: "Advertised as pgvector RAG but the live path is keyword matching. Honest labelling and a real embedding path would close the gap.",
    issues: [
      { severity: "high", text: "\"RAG / pgvector\" claim vs. actual keyword search.", fix: "Either wire real embeddings + vector search, or relabel as keyword search until it exists.", status: "pending" },
      { severity: "high", text: "Anon/RLS persistence gap.", fix: "Auth-bridge fix.", status: "pending" },
    ],
  },
  {
  ...
  },
  {
    ...
    target: 9.5,
    dims: { accuracy: 6, implementation: 6, execution: 6, results: 6 },
    verdict: "Good honest inventory of live/partial/planned, but the page itself lacked an auth guard and a few 'live' badges overstate reality.",
    issues: [
      { severity: "high", text: "Page had no auth guard (info about the deployment exposed to anon).", fix: "Add to middleware matcher / gate the route.", status: "pending" },
      { severity: "medium", text: "A few features badged 'live' are partial (e.g. internet-wide sift).", fix: "Downgrade the badges to match reality.", status: "pending" },
    ],
  },
  {
    page: "Settings",
